...
export async function POST(req: NextRequest) {
  ...
  const wa = `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP || "919876543210"}?text=${encodeURIComponent(`Namaste! Bargain deal LOCKED: ${s.product.name} (${s.product.sku}) at ₹${finalPrice}. Session: ${s.id.slice(0, 8)}`)}`;
