    ...
    <div>
      ...
      <div className="bg-white border rounded-2xl overflow-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((e) => <tr key={e.id} className="border-b"><td className="p-3">{e.name}<div className="text-xs text-gray-500">{e.business}</div></td><td className="p-3">{e.phone}</td><td className="p-3">{e.sku}</td><td className="p-3">{e.quantity}</td><td className="p-3"><EnquiryStatus id={e.id} status={e.status} /></td></tr>)}
